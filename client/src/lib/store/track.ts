...
export const useTrack = create<TrackState>()((set, get) => ({
  ...
  updateApplication: async (
    ...
    const filteredUpdates = Object.fromEntries(
      Object.entries(updates).filter(([key]) => key !== "thread_id" && key !== "application_stats")
    );

    // Call the UpdateApplication function
    const { data, error } = await UpdateApplication(
      profile,
      applicationId,
      filteredUpdates
    );
  ...
  fetchApplications: async (profile) => {
    ...
    const groupedApplications = data.reduce<
